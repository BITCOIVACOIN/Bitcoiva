...
chain_id = "bitcoiva-11001"
reconnect = true # true is the default
secret_key = "~/.tmkms/secret_connection.key"

[[providers.ledgertm]]
chain_ids = ["bitcoiva-11001"]
```

- Edit `addr` to point to your `bitcoivad` instance.
- Adjust `chain-id` to match your `.bitcoivad/config/config.toml` settings.
...
The last step is to retrieve the validator key that you will use in `bitcoivad`.
...
The KMS may complain that it cannot connect to bitcoivad. That is fine, we will fix it in the next section.
...
## Bitcoiva configuration

You need to enable KMS access by editing `.bitcoivad/config/config.toml`. In this file, modify `priv_validator_laddr` to create a listening address/port or a unix socket in `bitcoivad`.
...
Let's assume that you have set up your validator account and called it `kmsval`. You can tell bitcoivad the key that we've got in the previous section.

```bash
bitcoivad gentx --name kmsval --pubkey {.ValidatorKey} 
```

Now start `bitcoivad`. You should see that the KMS connects and receives a signature request.
