...
::: warning
**You need to [install bitcoiva](./installation.md) before you go further**
:::
...
```bash
bitcoivad init <your_custom_moniker>
```
...
You can edit this `moniker` later, in the `~/.bitcoivad/config/config.toml` file:
...
You can edit the `~/.bitcoivad/config/bitcoivad.toml` file in order to enable the anti spam mechanism and reject incoming transactions with less than the minimum gas prices:
...
Fetch the testnet's `genesis.json` file into `bitcoivad`'s config directory.

```bash
mkdir -p $HOME/.bitcoivad/config
curl https://raw.githubusercontent.com/cosmos/launch/master/genesis.json > $HOME/.bitcoivad/config/genesis.json
```
...
Your node needs to know how to find peers. You'll need to add healthy seed nodes to `$HOME/.bitcoivad/config/config.toml`. The [`launch`](https://github.com/cosmos/launch) repo contains links to some seed nodes.
...
Your full-node keeps unconfirmed transactions in its mempool. In order to protect it from spam, it is better to set a `minimum-gas-prices` that the transaction must meet in order to be accepted in your node's mempool. This parameter can be set in the following file `~/.bitcoivad/config/bitcoivad.toml`.
...
Bitcoiva can dump the entire application state to a JSON file, which could be useful for manual analysis and can also be used as the genesis file of a new network.
...
```bash
bitcoivad start --assert-invariants-blockly
```
...
invariant broken:
    ...
    CRITICAL please submit the following transaction:
        bitcoivacli tx crisis invariant-broken staking supply
