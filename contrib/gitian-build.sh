...
DEFAULT_BITCOIVA_SIGS=${BITCOIVA_SIGS:-'bitcoiva.sigs'}
DEFAULT_GITIAN_REPO='https://github.com/devrandom/gitian-builder'
DEFAULT_GBUILD_FLAGS=''
DEFAULT_SIGS_REPO='https://github.com/cosmos/bitcoiva.sigs'
...
The signature will be saved in '${DEFAULT_BITCOIVA_SIGS}/'. An alternative output directory
for signatures can be supplied via the environment variable \$BITCOIVA_SIGS.
...
f_build() {
  ...
  bin/gbuild --commit bitcoiva="$g_commit" ${GBUILD_FLAGS} "$l_descriptor"
...
g_sigs_dir=${BITCOIVA_SIGS:-"${g_workdir}/${DEFAULT_BITCOIVA_SIGS}"}
